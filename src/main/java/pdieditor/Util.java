...
public class Util {
	...
	public static PlanarImage grayscale(PlanarImage pi) {
		...
		double[][] matrix;
		switch (pi.getSampleModel().getNumBands()) {
			case 3:
				double[][] matrix3 = {{0.3D, 0.59D, 0.11D, 0D}};
				matrix = matrix3;
				break;
			case 4:
				double[][] matrix4 = {{0.3D, 0.59D, 0.11D, 0D, 0D}};
				matrix = matrix4;
				break;
			case 1:
				double[][] matrix1 = {{1D, 0D}};
				matrix = matrix1;
				break;
			default:
				return null;
	...
	public static PlanarImage gerarGrafico(Histogram h) {
		...
		for (int i = 0; i < aux.length; i++) {
			String s = Integer.toString(8 * (int) (i / 8));
